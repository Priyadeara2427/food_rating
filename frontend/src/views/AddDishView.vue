      ...
      <form @submit.prevent="submitForm" class="dish-form">
        <div class="form-group">
          ...
          <select 
            ...
            @blur="validateField('canteenId')"
            :class="{ 'error': errors.canteenId }"
            required
          >
          ...
          <span v-if="errors.canteenId" class="error-message">{{ errors.canteenId }}</span>
        </div>
...
export default {
  ...
  mounted() {
    this.fetchCanteens()
    // Pre-select canteen if coming from canteen page
    if (this.$route.query.canteenId) {
      this.form.canteenId = this.$route.query.canteenId
    }
  },
  methods: {
    async fetchCanteens() {
      try {
        ...
        this.showError('Failed to load canteens')
      }
    },
    validateField(field) {
      this.errors[field] = ''
      ...
      switch(field) {
        case 'canteenId':
          if (!this.form.canteenId) this.errors.canteenId = 'Please select a canteen'
          break
        case 'name':
